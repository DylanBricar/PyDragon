...
class KamehouseIn:
    ...
    def while_kamehouse_in(self):
        ...
        collision_tortue = tilemap.layers['evenements'].find('collision_tortue')  # Récupère toutes les collisions
        ...
        txt_mission01_01 = ("Hey Son Goku, j'espère que tu vas bien ça faisait longtemps !",
        ...
        txt_class = Interact(self.screen)  # Définission de la classe de dialogue par défaut
        ...
        while self.while_map_kamehouse_in:  # Boucle infinie du jeu
            ...
            collide_tortue = CollisionController(player, collision_tortue)  # Appelle la class de collision pour parler avec Tortue Géniale
            ...
            for event in pygame.event.get():  # Vérifie toutes les actions du joueur
                if event.type == pygame.QUIT:  # Clique pour quitter le jeu
                    ...
                    Niveau.DIALOGUE = False  # On défini la variable pour la cacher
            ...
            if Niveau.DIALOGUE:  # Si la variable de dialogue est définie
                txt_class.show_box(txt_mission01_01, 'tortue')  # Affiche le dialogue pour la mission 01
